...
KRITITVA_RATE_LIMIT_ENABLED=true            # per-org RPS guard (NFR-5.2.5); no-ops without Redis
KRITITVA_ORG_RPS=100                         # max requests per org per window
KRITITVA_RATE_LIMIT_WINDOW_S=1               # window length; org_rps applies per this many seconds
